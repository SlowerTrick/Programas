...
{
    int produto_existe = busca_produto_lista(RetornaNome(prod), pedido->produtos);

    if(!produto_existe)
    {
        adiciona_lista(prod, pedido->produtos);
        return;
    }
...
{
    printf("Dono: %s", pedido->dono);
    imprime_lista(pedido->produtos);
}
...
{
    remove_lista(prod, pedido->produtos);
}
...
{
    int calorias_totais = retorna_calorias_totais(pedido->produtos);
    TProduto *verifica_restricao = verifica_ingrediente_lista(pedido->produtos, restricao_alimentar);

    if(calorias_totais > restricao_calorica)     
    {
        printf("Pedido não Enviado! Pedido tem mais calorias do que a restricao, tente retirar algum produto!");
        return 0;
    }
    if(verifica_restricao != NULL)
    {
        printf("Pedido não Enviado! Restricao alimentar no produto: %s\n", RetornaNome(verifica_restricao));
        return 0;
    }

    if(pedido)
    {
        libera_lista(pedido->produtos);
        free(pedido->dono);
        free(pedido);
        return 1;
    }

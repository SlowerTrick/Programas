...
int EhValido(tPacote pacote)
{
    int idmsg = 0, idpct = 0, conteudo = 0, codigoErro = 0;
    sscanf(pacote.idMsg, "%d", &idmsg);
    sscanf(pacote.idPct, "%d", &idpct);
    conteudo = pacote.tamanhoConteudo - '0';
    int soma = idmsg + idpct + conteudo;
    ...
    sscanf (pacote.codigoErro, "%d", &codigoErro);
